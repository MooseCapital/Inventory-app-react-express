...
import {persistAxiosData} from "../../Hooks/persistFetch.js";
...
function Home(props) {
    ...
    */
    ...
    const [persistComponent, setPersistComponent, updateData] = persistAxiosData('/store/test')
    ...
    return (
        <>
            ...
            {
                persistComponent.loading ? <div>loading...</div> :
                    <>
                        <div>test page</div>
                        <div>session storage solution with local component state, no props</div>
                        <div>{JSON.stringify(persistComponent.fetchData)}</div>
                    </>
            }
        </>
    )
